...
impl<'src> GoConverter<'src> {
  ...
  pub fn get_tmp_var(&mut self, t: Option<il::Type>) -> (il::Value, String) {
    ...
    (
      il::Value {
        kind: il::ValueKind::Var {
          name: name.clone(),
          t: t.unwrap_or(il::Type::Any),
        },
        tag: None,
      },
      name,
    ...
    match param {
              ...
              .map(|name| {
                if BUILTIN_TYPES.contains_key(name.as_str()) {
                  il::Value {
                    kind: il::ValueKind::Var {
                      name: "_".to_string(),
                      t: BUILTIN_TYPES[name.as_str()].clone(),
                    },
                    tag: None,
                  }
                } else {
                  il::Value {
                    kind: il::ValueKind::Var { name, t: t.clone() },
                    tag: None,
                  }
                }
              })
      ...
      ParamDecl::VariadicParameterDeclaration(vd) => {
        ...
        Ok(vec![il::Value {
          kind: il::ValueKind::Var {
            name,
            t: il::Type::Array(Box::new(t.clone()), None),
          },
          tag: None,
        }])
      }
      ...
      SimpleType_ParameterList::ParameterList(pl) => {
        self.convert_parameter_list(root, &pl).map(|vs| {
          vs.into_iter()
            .map(|v| match v {
              il::Value {
                kind: il::ValueKind::Var { name: _, t },
                tag: _,
              } => t,
              _ => {
                eprintln!("Expected Var value in function return type, got: {:?}", v);
                il::Type::Any
              }
            })
            .collect::<Vec<_>>()
    ...
    match node {
      ...
      Expression::FuncLiteral(fl) => {
        ...
        let params = params
          .iter()
          .map(|p| match &p.kind {
            il::ValueKind::Var { name, t } => Ok((t.clone(), Some(name.clone()))),
      ...
      Expression::CompositeLiteral(cl) => {
        ...
        Ok((
          il::Value {
            kind: il::ValueKind::CompositeLit(type_, Box::new(elems)),
            tag: Some(tag),
          },
          stmts,
        ))
