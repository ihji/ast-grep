...
fn annotate_stmt(
  ...
  match stmt {
    ...
    il::Statement::Assign { left, right, .. } => {
      annotate_value(ctx, parent_scope_id, left);
      annotate_value(ctx, parent_scope_id, right);
    }
    _ => {}
  }
}
...
fn annotate_value(
  ...
  match &mut value.kind {
    il::ValueKind::Ident(name) => {
      if let Some(symbol) = ctx.lookup_symbol(name, Namespace::Value, parent_scope_id) {
...
fn annotate_expr(ctx: &mut NamingContext, parent_scope_id: Option<ScopeId>, expr: &mut il::Expr) {
  match expr {
    il::Expr::UnOp { value, .. } => {
      annotate_value(ctx, parent_scope_id, value);
    }
    ...
    il::Expr::DotAccess { base, .. } => {
      annotate_value(ctx, parent_scope_id, base);
    }
    _ => {}
  }
}
